<nav x-data="{ open: false, logoutConfirmOpen: false }" class="bg-white border-b border-gray-100 relative z-[100]">
                ...
                <div class="relative mr-3">
                    <x-dropdown align="right" width="w-[450px]">
                        ...
                        <x-slot name="content">
                            ...
                            <!-- Footer -->
                            <a href="{{ $isBukuSaku ? url('/buku-saku/history') : url('/history') }}" class="block w-full py-3 text-center text-xs font-semibold text-gray-500 hover:text-blue-600 hover:bg-gray-50 border-t border-gray-100 transition-colors">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="url('/integrasi-sistem')" :active="request()->routeIs('integrasi-sistem.*')">
                {{ __('Integrasi Sistem') }}
            </x-responsive-nav-link>
            @endcan

            @can('view module management-user')
            <x-responsive-nav-link :href="url('/management-user')" :active="request()->routeIs('management-user*')">
                {{ __('Management User') }}
            </x-responsive-nav-link>
            @endcan

            @can('view module history')
            <x-responsive-nav-link :href="url('/history')" :active="request()->routeIs('history')">
                {{ __('History Aktivitas') }}
            </x-responsive-nav-link>
            @endcan
        </div>
