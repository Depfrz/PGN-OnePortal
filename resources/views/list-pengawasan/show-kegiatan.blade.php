<x-dashboard-layout title="Detail Kegiatan" :can-write="$canWrite" :lp-permissions="$lpPermissions">
    <div x-data="{
        ...
        editProject: { 
            nama: {{ Js::from($item['nama'] ?? '') }}, 
            deskripsi: {{ Js::from($item['deskripsi'] ?? '') }},
            status: {{ Js::from(($item['status'] == 'Belum Dimulai' ? 'Belum Dikerjakan' : $item['status']) ?? 'Belum Dikerjakan') }}
        },
        ...
        savingAll: false,
        fileErrorModal: false,
        validateFile(file) {
            if (file.size > 10 * 1024 * 1024) { // 10MB
                this.fileErrorModal = true;
                return false;
            }
            return true;
        },
        ...
        async submitAll() {
            ...
            try {
                if (canSaveDetail) {
                    ...
                    this.project.status = detailPayload.status;
                }
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (idx !== -1) {
                        ...
                        let updatedKeterangan = JSON.parse(JSON.stringify(this.project.keterangan));
                        updatedKeterangan[idx].bukti = data.bukti;
                        this.project.keterangan = updatedKeterangan;
                    }
        ...
        async onKeteranganBuktiChange(label, e) {
            ...
            await this.uploadKeteranganBukti(label, file);
            e.target.value = '';
        },
        ...
        <div class="space-y-6">
                <div class="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            <div class="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1">Status</div>
                            <select x-model="editProject.status" :disabled="!canWrite || !lpPerms.tambah_kegiatan" class="w-full bg-gray-50 border border-gray-300 rounded-lg px-4 py-2.5 text-sm text-gray-900 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none disabled:opacity-60 dark:bg-gray-900 dark:border-gray-700 dark:text-gray-100">
                ...
                <div class="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5">
                    <div class="flex items-center justify-between mb-4">
                        <div class="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider">
                            Bukti Kegiatan
                            <span class="text-[10px] text-gray-500 dark:text-gray-400 font-normal normal-case ml-1">(Max 10MB)</span>
                        </div>
                                ...
                                <div x-show="canWrite && lpPerms.bukti" class="mt-auto pt-3 border-t border-gray-100 dark:border-gray-700">
                                    <template x-if="!getKeteranganBukti(opt)">
                                        <label class="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-blue-600 text-white font-medium hover:bg-blue-700 transition-colors cursor-pointer text-xs w-full justify-center sm:w-auto sm:justify-start" @click.stop>
                                            ...
                                            Upload Bukti <span class="opacity-75 text-[10px] ml-1">(Max 10MB)</span>
        ...
        <div x-show="fileErrorModal" class="fixed inset-0 z-[10000] flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm transition-opacity" style="display: none;">
            <div class="bg-white rounded-xl p-5 w-[90vw] max-w-[320px] shadow-2xl transform transition-all dark:bg-gray-800 border border-red-500/20">
                <div class="flex flex-col items-center text-center">
                    <div class="h-12 w-12 bg-red-100 rounded-full flex items-center justify-center mb-3 dark:bg-red-900/30">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-red-600 dark:text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                        </svg>
                    </div>
                    <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-1">File Terlalu Besar</h3>
                    <p class="text-gray-500 dark:text-gray-400 mb-4 text-xs leading-relaxed">
                        Ukuran file melebihi batas <strong>10MB</strong>.<br>Silakan pilih file lain.
                    </p>
                    <button @click="fileErrorModal = false" class="w-full px-4 py-2 bg-red-600 text-white text-sm rounded-lg hover:bg-red-700 font-semibold shadow-md shadow-red-500/20 transition-all">
                        Mengerti
                    </button>
                </div>
            </div>
