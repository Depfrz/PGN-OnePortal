<x-buku-saku-layout title="Dokumen Kedaluwarsa">
    <div class="mb-4">
        <h2 class="text-lg font-bold text-gray-800">Dokumen Kedaluwarsa</h2>
        <p class="text-gray-500 text-xs">Daftar dokumen yang masa berlakunya hampir habis (< 1 tahun) atau sudah kedaluwarsa.</p>
    </div>

    <!-- Search Section -->
    <div class="bg-white p-4 sm:p-6 rounded-lg shadow-sm mb-6">
        <h3 class="text-lg font-semibold text-gray-800 mb-3">Pencarian Dokumen Kedaluwarsa</h3>
        <form x-data="{ 
            open: false, 
        ...
        }" action="{{ url('/buku-saku/expired') }}" method="GET" class="flex flex-col md:flex-row gap-4">
            <!-- Search Input -->
            <div class="flex-1 relative">
                ...
                <input type="text" name="q" x-model="query" 
                    ...
                    placeholder="Cari dokumen kedaluwarsa...">
            </div>
    ...
    @if($documents->isEmpty())
        <div class="text-center py-10 text-gray-500">
            <p>Tidak ditemukan dokumen kedaluwarsa atau yang mendekati kedaluwarsa.</p>
        </div>
    @else
            ...
            @foreach($documents as $doc)
                <div class="border rounded-lg p-4 hover:shadow-md transition-shadow flex items-start justify-between bg-white gap-4">
                    <div class="flex items-start gap-4 flex-1 min-w-0">
                        <div class="flex-1 min-w-0">
                            <h3 class="font-bold text-base text-gray-800 break-words mb-1">
                                <a href="{{ url('/buku-saku/preview/' . $doc->id) }}" target="_blank" rel="noopener noreferrer" class="hover:text-blue-600 hover:underline">
                                    {{ $doc->title }}
                                </a>
                            </h3>
                            ...
                            <div class="text-sm font-bold mt-1">
                                @if($doc->valid_until)
                                    ...
                                    @elseif($diffInYears <= 1 && $diffInYears > 0)
                                        <span class="text-red-600">Sisa Waktu: <span class="countdown-timer" data-target="{{ $doc->valid_until }}">{{ $now->diff($doc->valid_until)->format('%m Bln %d Hr') }}</span></span>
                                    @else
                                        <span class="text-red-600">Sudah Kedaluwarsa ({{ \Carbon\Carbon::parse($doc->valid_until)->translatedFormat('d F Y') }})</span>
                                    @endif
                                @else
                    ...
                    <div class="flex items-center gap-1.5 flex-shrink-0">
                        ...
                        <form action="{{ url('/buku-saku/favorites/' . $doc->id) }}" method="POST">
                            @csrf
                        ...
                        <!-- Download Button -->
                        <a href="{{ url('/buku-saku/download/' . $doc->id) }}" class="p-1.5 rounded-full hover:bg-gray-100 text-blue-600" title="Unduh">
