<x-buku-saku-layout>
    <div class="mb-6">
        <a href="{{ url('/buku-saku') }}" class="text-blue-600 hover:text-blue-800 flex items-center gap-2">
    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden h-[80vh] flex flex-col">
        <div class="p-4 border-b flex flex-col sm:flex-row justify-between items-start sm:items-center bg-gray-50 gap-4">
            ...
            <div class="flex items-center gap-2 w-full sm:w-auto">
                <a href="{{ url('/buku-saku/download/' . $document->id) }}" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 text-sm font-medium text-center w-full sm:w-auto">
                    Download
                </a>
            </div>
        </div>
        <div class="flex-1 bg-gray-100 p-4">
            @if(in_array(strtolower($document->file_type), ['pdf']))
                <iframe src="{{ url('/buku-saku/preview/' . $document->id) }}" class="w-full h-full rounded border bg-white"></iframe>
            @elseif(in_array(strtolower($document->file_type), ['jpg', 'jpeg', 'png', 'gif']))
                <div class="flex items-center justify-center h-full">
                    <img src="{{ url('/buku-saku/preview/' . $document->id) }}" alt="{{ $document->title }}" class="max-w-full max-h-full rounded shadow">
                </div>
            @else
                <div class="flex flex-col items-center justify-center h-full text-gray-500">
                    ...
                    <a href="{{ url('/buku-saku/download/' . $document->id) }}" class="mt-4 text-blue-600 hover:underline">Download File</a>
                </div>
            @endif
        </div>
    </div>
