    ...
    <div x-data="{
        ...
        openEditAccess(user) {
            ...
            this.listPengawasanPermissions = {
                ...
                status_proyek: isFullAccessRole,
                bulk_proyek: isFullAccessRole,
                tambah_kegiatan: isFullAccessRole,
                hapus_kegiatan: isFullAccessRole,
                bulk_kegiatan: isFullAccessRole,
                filter_status_kegiatan: isFullAccessRole,
                kelola_jenis_pekerjaan: isFullAccessRole,
                kelola_template_bulk: isFullAccessRole,
        ...
        async saveUser() {
            try {
                const response = await fetch('{{ url('/management-user') }}', {
                    method: 'POST',
                    headers: {
                ...
                if (response.ok) {
                    ...
                    try {
                        ...
                        this.showError('Gagal menambahkan user: ' + (data.message || 'Unknown error'));
                    } catch (e) {
                        this.showError('Gagal: Terjadi kesalahan server (500).');
                    }
                }
            } catch (error) {
                console.error(error);
                this.showError('Terjadi kesalahan sistem: ' + error.message);
            }
        },
        ...
        async saveRole() {
            try {
                ...
                if (response.ok) {
                    ...
                    this.showError('Gagal memperbarui role: ' + (data.message || response.statusText));
                }
            } catch (error) {
                console.error(error);
                this.showError('Terjadi kesalahan sistem: ' + error.message);
        ...
        async saveAccess() {
            try {
                ...
                if (response.ok) {
                    ...
                    try {
                        const data = JSON.parse(text);
                        let errorMsg = data.message || 'Gagal memperbarui hak akses';
                        if (data.errors) {
                            errorMsg += ' (' + Object.values(data.errors).flat().join(', ') + ')';
                        }
                        this.showError('Gagal: ' + errorMsg);
                    } catch (e) {
                        console.error('Non-JSON error response:', text);
                        this.showError('Gagal: Terjadi kesalahan server (500). Cek console.');
                    }
                }
            } catch (error) {
                console.error(error);
                ...
                <div class="space-y-4">
                    ...
                    <div class="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
                        @php
                            ...
                            $subModules = ['Beranda', 'Dokumen Favorit', 'Pengecekan File', 'Dokumen Kedaluwarsa', 'Riwayat Dokumen', 'Upload Dokumen'];
                            $accessDescriptions = [
                                'Buku Saku' => [
                                    ...
                                    'Dokumen Kedaluwarsa' => 'Melihat dokumen yang sudah atau akan kedaluwarsa.',
                        ...
                        @foreach($finalGroupOrder as $groupName)
                            @php
                                ...
                                // Sort Item Modules if they are in subModules list
                                if ($groupName === 'Buku Saku') {
                                    $itemModules = $itemModules->sortBy(function($module) use ($subModules) {
                                        $index = array_search($module->name, $subModules);
                                ...
                                @if($groupName === 'List Pengawasan')
                                    <div class="mt-4">
                                        ...
                                        <div class="mb-3 text-xs text-gray-500 dark:text-gray-400">
                                            Fitur List Pengawasan yang tidak diberi akses akan disembunyikan, dan user tidak bisa berinteraksi dengan fitur tersebut.
                                        </div>
                                        <div class="grid grid-cols-1 gap-3 max-h-[400px] overflow-y-auto pr-2">
                                            ...
                                            <label class="flex items-center gap-4 cursor-pointer p-3 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:border-blue-200 dark:hover:border-blue-800 transition-colors">
                                                <input type="checkbox" x-model="listPengawasanPermissions.status_proyek" class="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                                                <div class="flex flex-col">
                                                    <span class="text-sm font-semibold text-gray-800 dark:text-gray-200">Status Proyek</span>
                                                    <span class="text-xs text-gray-500 dark:text-gray-400">Melihat kolom status, filter status, dan mengubah status proyek.</span>
                                                </div>
                                            </label>
                                            <label class="flex items-center gap-4 cursor-pointer p-3 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:border-blue-200 dark:hover:border-blue-800 transition-colors">
                                                <input type="checkbox" x-model="listPengawasanPermissions.bulk_proyek" class="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                                                <div class="flex flex-col">
                                                    <span class="text-sm font-semibold text-gray-800 dark:text-gray-200">Bulk Proyek</span>
                                                    <span class="text-xs text-gray-500 dark:text-gray-400">Memilih banyak proyek untuk ubah status dan hapus secara massal.</span>
                                                </div>
                                            </label>
                                            ...
                                            <label class="flex items-center gap-4 cursor-pointer p-3 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:border-blue-200 dark:hover:border-blue-800 transition-colors">
                                                <input type="checkbox" x-model="listPengawasanPermissions.bulk_kegiatan" class="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                                                <div class="flex flex-col">
                                                    <span class="text-sm font-semibold text-gray-800 dark:text-gray-200">Bulk Kegiatan</span>
                                                    <span class="text-xs text-gray-500 dark:text-gray-400">Tambah kegiatan bulk dari template dan ubah status kegiatan massal.</span>
                                                </div>
                                            </label>
                                            <label class="flex items-center gap-4 cursor-pointer p-3 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:border-blue-200 dark:hover:border-blue-800 transition-colors">
                                            ...
                                            <label class="flex items-center gap-4 cursor-pointer p-3 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:border-blue-200 dark:hover:border-blue-800 transition-colors">
                                                <input type="checkbox" x-model="listPengawasanPermissions.filter_status_kegiatan" class="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                                                <div class="flex flex-col">
                                                    <span class="text-sm font-semibold text-gray-800 dark:text-gray-200">Filter Status Kegiatan</span>
                                                    <span class="text-xs text-gray-500 dark:text-gray-400">Menampilkan tombol filter status di daftar kegiatan.</span>
                                                </div>
                                            </label>

                                            <!-- Group: Master Data -->
                                            <div class="font-medium text-gray-700 dark:text-gray-300 mt-2 mb-1">Master Data</div>
                                            <label class="flex items-center gap-4 cursor-pointer p-3 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:border-blue-200 dark:hover:border-blue-800 transition-colors">
                                                <input type="checkbox" x-model="listPengawasanPermissions.kelola_jenis_pekerjaan" class="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                                                <div class="flex flex-col">
                                                    <span class="text-sm font-semibold text-gray-800 dark:text-gray-200">Kelola Jenis Pekerjaan</span>
                                                    <span class="text-xs text-gray-500 dark:text-gray-400">Tambah, edit, dan hapus jenis pekerjaan.</span>
                                                </div>
                                            </label>
                                            <label class="flex items-center gap-4 cursor-pointer p-3 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:border-blue-200 dark:hover:border-blue-800 transition-colors">
                                                <input type="checkbox" x-model="listPengawasanPermissions.kelola_template_bulk" class="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                                                <div class="flex flex-col">
                                                    <span class="text-sm font-semibold text-gray-800 dark:text-gray-200">Kelola Template Bulk</span>
                                                    <span class="text-xs text-gray-500 dark:text-gray-400">Tambah, edit, dan hapus template kegiatan per jenis pekerjaan.</span>
                                                </div>
                                            </label>
