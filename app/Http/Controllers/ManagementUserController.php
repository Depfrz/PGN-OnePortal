...
class ManagementUserController extends Controller
{
    public function index()
    {
        ...
        $availableAccess = Module::orderBy('group')->orderBy('order')->orderBy('name')->get()->groupBy('group');
    ...
    public function updateAccess(Request $request, User $user)
    {
        ...
        $request->validate([
            'hak_akses' => ['array'],
            // 'hak_akses.*' => ['exists:modules,name'], // Disabled to prevent validation issues with new modules
        ...
        try {
            ...
            Log::error('Failed to update access for user ' . $user->id . ': ' . $e->getMessage());
    ...
    private function syncAccess(User $user, array $moduleNames, array $dashboardModuleNames = [], array $listPengawasanPermissions = [])
    {
        ...
        $subModules = ['Dokumen Favorit', 'Riwayat Dokumen', 'Pengecekan File', 'Upload Dokumen', 'Beranda', 'Dokumen Kedaluwarsa'];
