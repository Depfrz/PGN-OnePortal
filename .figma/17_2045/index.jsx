import React from 'react';

import styles from './index.module.scss';

const Component = () => {
  return (
    <div className={styles.adminMengeklikTambah}>
      <div className={styles.halamanAdmin}>
        <div className={styles.rectangle5}>
          <img
            src="../image/mkmaknz3-zopdnv3.png"
            className={styles.logoPertaminaGasNega}
          />
          <div className={styles.autoWrapper}>
            <div className={styles.lineMdHomeTwotone}>
              <img src="../image/mkmaknz1-3lbtrd6.svg" className={styles.vector} />
              <img src="../image/mkmaknz1-u0tppbq.svg" className={styles.group} />
              <img
                src="../image/mkmaknz1-n7fsvjh.svg"
                className={styles.materialSymbolsHomeR}
              />
            </div>
            <p className={styles.beranda}>Beranda</p>
          </div>
          <div className={styles.autoWrapper2}>
            <div className={styles.materialSymbolsHisto2}>
              <img
                src="../image/mkmaknz0-n68ei2b.svg"
                className={styles.materialSymbolsHisto}
              />
            </div>
            <p className={styles.history}>History</p>
          </div>
          <div className={styles.rectangle23}>
            <div className={styles.rectangle24}>
              <div className={styles.majesticonsUsers}>
                <img
                  src="../image/mkmaknz0-7g9lqoq.svg"
                  className={styles.materialSymbolsHisto}
                />
              </div>
              <p className={styles.managamentUser}>Managament User</p>
            </div>
          </div>
          <p className={styles.integrasiSistem}>Integrasi Sistem</p>
        </div>
        <div className={styles.autoWrapper3}>
          <div className={styles.rectangle6}>
            <p className={styles.berandaManagementUse}>Beranda →Management User</p>
            <div className={styles.ionNotifcations}>
              <img src="../image/mkmaknz0-2oka9nm.svg" className={styles.frame1} />
            </div>
            <p className={styles.admin}>Admin</p>
            <img src="../image/mkmaknz0-69n6zod.svg" className={styles.ggProfile} />
          </div>
          <div className={styles.rectangle9}>
            <p className={styles.pencarianUser}>Pencarian User</p>
            <div className={styles.rectangle10}>
              <p className={styles.cariNama}>Cari Nama ......</p>
            </div>
          </div>
          <div className={styles.rectangle11}>
            <p className={styles.manajemenUser}>Manajemen User</p>
            <div className={styles.rectangle12} />
          </div>
        </div>
      </div>
      <p className={styles.beranda2}>Beranda</p>
      <div className={styles.rectangle27}>
        <div className={styles.autoWrapper6}>
          <div className={styles.autoWrapper4}>
            <p className={styles.formUser}>Form User</p>
            <p className={styles.nama}>Nama</p>
          </div>
          <div className={styles.autoWrapper5}>
            <p className={styles.password}>Password</p>
            <div className={styles.rectangle31} />
          </div>
        </div>
        <div className={styles.autoWrapper7}>
          <div className={styles.rectangle31} />
          <p className={styles.jabatan}>Jabatan</p>
        </div>
        <div className={styles.autoWrapper8}>
          <p className={styles.email}>Email</p>
          <div className={styles.rectangle31} />
        </div>
        <div className={styles.autoWrapper9}>
          <div className={styles.rectangle31} />
          <p className={styles.role}>Role</p>
        </div>
        <div className={styles.rectangle33} />
        <div className={styles.autoWrapper12}>
          <div className={styles.autoWrapper10}>
            <p className={styles.password}>Instansi</p>
            <div className={styles.rectangle31} />
          </div>
          <div className={styles.autoWrapper11}>
            <p className={styles.password}>Hak</p>
            <div className={styles.rectangle31} />
          </div>
        </div>
        <div className={styles.autoWrapper13}>
          <div className={styles.rectangle35} />
          <p className={styles.simpan}>Simpan</p>
        </div>
      </div>
      <p className={styles.history2}>History</p>
      <p className={styles.managamentUser2}>Managament User</p>
      <div className={styles.autoWrapper14}>
        <p className={styles.integrasiSistem2}>Integrasi Sistem</p>
        <p className={styles.batal}>Batal</p>
      </div>
      <div className={styles.vector3}>
        <img src="../image/mkmaknz1-537w73p.svg" className={styles.vector2} />
      </div>
      <p className={styles.batal2}>Batal</p>
    </div>
  );
}

export default Component;
